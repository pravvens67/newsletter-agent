...
class NewsletterAgent:
    ...
    def authenticate_gmail(self, email_address):
        """Robust, cross-platform Gmail OAuth: supports Streamlit Cloud & local"""
        try:
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    try:
                        ...
                        creds = None
                # If token missing or cannot refresh: interactive OAuth
                if not creds:
                    if not creds_path or not os.path.exists(creds_path):
                        st.error("Credentials file not found. Add your Gmail API credentials to Streamlit Secrets.")
                        return None
                    flow = InstalledAppFlow.from_client_secrets_file(creds_path, SCOPES)
                    # Try run_console (copy-paste flow), fall back to local flow or manual token upload
                    if hasattr(flow, "run_console"):
                        st.info("Copy the URL below, authorize in your browser, and paste the code back here:")
                        creds = flow.run_console()
                    else:
                        st.error(
                            "Cannot open authentication page in this environment. "
                            "Solution: run this app on your computer once to generate a /tokens/ file, then upload that token file to the deployed app."
                        )
                        return None
                # Cache token
                try:
